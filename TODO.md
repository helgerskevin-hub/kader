...
- [ ] `scoorCandles()` gespiegeld voor short: stop net boven de recente swing high in plaats van onder de swing low, doel onder de entry. `stopAfstandStructuur()` krijgt daarvoor een richtingparameter.
  - **Besloten 26 aug 2026 (Thom): geen nieuw scoreprofiel.** Deze regel zei eerder "scoring op afwaartse trend", maar dat is nooit gemeten. Meting F gebruikte de BESTAANDE momentum-score met `score < 40`, en dat is het enige waar cijfers onder liggen. We bouwen dus wat gemeten is, net als bij fase 3: alleen de niveaus worden gespiegeld, de score blijft zoals hij is. Let op bij het bouwen: meting F draaide zonder R/R-filter, en doel 2x ATR met de stop-clamp van 0,5x tot 3x ATR geeft een R/R tussen 0,67 en 4,0, dus `MIN_RISK_REWARD` van 2,0 zou een deel wegfilteren. Voorstel is een `STOP_CAP_SHORT` van 1,0 (zelfde truc als `STOP_CAP_OMKEER`), waarmee de R/R per constructie tussen 2,0 en 4,0 ligt. Dat wijzigt de gemeten strategie, dus meting F moet daarna opnieuw draaien en de jaarcijfers moeten reproduceren.
  - `shortNiveaus()` in [backtest.ts](app/scripts/backtest.ts) is al een werkende spiegel van `stopAfstandStructuur()` met een eigen `SWING = 10` ernaast. Die duplicaat opheffen en de backtest de engine-functie laten importeren, anders meet de backtest een andere implementatie dan de app draait.
  - `poortOpen()` heeft een spiegel nodig die alleen bij ONGUNSTIG opengaat. Let op het null-gedrag: zonder klimaatdata staat de long-poort open ("geen data is geen reden om te blokkeren"), maar een short zonder klimaatdata is over negen jaar gemiddeld negatief, dus daar hoort de poort juist dicht te blijven.
